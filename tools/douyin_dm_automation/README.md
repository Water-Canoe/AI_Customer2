...
打开 `http://127.0.0.1:8025/` 后填写用户主页 URL 和话术。首次运行会弹出浏览器；如果抖音提示未登录，就在该浏览器里完成登录，脚本最长等待 300 秒。抖音页面会渲染一个隐藏的“私信”按钮副本，脚本只匹配文本严格等于“私信/发私信”的可见入口，并使用 Playwright Locator 等待按钮位置稳定后真实点击；首次点击未打开聊天框时会重新定位并最多重试 3 次，进入聊天路由后只等待输入框。
