...
第一版只实际执行抖音；计划工作台仍展示抖音、快手、小红书平台入口，点击快手或小红书只提示“正在开发”。动作组合包括点赞视频、收藏视频、关注作者、评论文案、评论图片；一个动作都不选时允许启动，表示“纯自动刷视频”，只随机停留、切换视频并写入“仅浏览”记录。文案和图片在引流设置页维护，文案库使用可增删改的列表编辑器，执行时随机抽取。
