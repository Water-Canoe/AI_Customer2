...
账号主页简介不是所有采集模式都会带回。抖音关键词搜索当前常见结果只会写内容作者字段，`user_signature` 可能为空。总览树账号行的“账号分析”会创建 `account_analysis` 任务，使用 MediaCrawler creator 模式补采主页资料和少量视频；导入时以 `dy_creator.desc` / `xhs_creator.desc` 作为主页简介权威来源，并把少量视频写入内容库。采集成功后系统会把账号资料和最近视频打包发送给 AI，写回 `competitor_status`：竞品账号显示“找客户”和“删除”，非竞品可在账号行单独删除，也可在关键词行用“一键删除非竞品”批量清理。
