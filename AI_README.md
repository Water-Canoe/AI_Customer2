...
执行中参考 `GHkmmm/laizan` 的可靠做法监听 `aweme/v1/web/tab/feed/` 和详情接口响应，用活跃视频 ID 反查作者、简介、点赞数和评论数。互动按钮只在当前活跃视频容器或当前详情视频容器内查找，不使用全局第一个“关注/点赞”按钮。评论文案按整条 `insert_text` 写入，并在发送前校验输入框内容必须等于文案库原文；评论图片必须等待上传预览；评论发送必须捕获 `aweme/v1/web/comment/publish` 成功响应后才写成功记录。点赞、收藏、关注只有在当前视频控件状态确认变化后才写入防重复账本。
