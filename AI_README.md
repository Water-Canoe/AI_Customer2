...
执行中参考 `GHkmmm/laizan` 的可靠做法监听 `aweme/v1/web/tab/feed/` 和详情接口响应，用活跃视频 ID 反查作者、简介、点赞数和评论数。互动按钮只在当前活跃视频容器或当前详情视频容器内查找，不使用全局第一个“关注/点赞”按钮。评论文案按整条 `insert_text` 写入，并在发送前校验输入框内容必须等于文案库原文；评论图片必须等待上传预览；评论发送优先点击评论区“发布/发送”按钮，并捕获 `aweme/v1/web/comment/publish` 成功响应后才写成功记录；无论评论成功或失败，切换下一条前都会先关闭评论区，避免输入框吞掉下滑快捷键。点赞优先点击当前视频按钮；只在没有找到按钮时才用 `z` 快捷键兜底，避免按钮点击后再次按快捷键把赞取消。收藏、关注只有在当前视频控件状态确认变化后才写入防重复账本。
