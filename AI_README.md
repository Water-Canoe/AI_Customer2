...
引流设置页右栏提供“打开抖音登录窗口”按钮，调用 `POST /api/traffic/douyin-login`。后端会启动一个独立 Playwright 登录窗口，并复用执行器同一个 `runtime/traffic_douyin_profile`；用户扫码后可以确认首页能正常进入视频流，后续随机引流批次会从抖音主页开始随机点击页面上的视频卡片，不复用拓客项目库视频。
