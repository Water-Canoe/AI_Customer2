...
广告、直播、图文和其它非常规视频会写入 `skipped` 记录并自动切换下一条，不算批次失败。页面 `goto` 超时会先继续检查当前页面，不会仅因加载完成信号缺失就停机；单个视频互动动作遇到 Playwright 超时会重试一次，连续 2 次仍失败且没有任何动作成功时记录“跳过”并下滑到下一条。每日动作上限只统计真实互动动作，纯刷视频不消耗动作额度。防重复按动作分作用域：点赞/收藏/评论按视频去重，关注按作者去重，评论额外按文案和图片 hash 去重。
