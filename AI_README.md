...
左侧栏底部提供“检查更新”按钮，并显示后端 `GET /api/health` 返回的当前程序版本。点击后前端调用 `POST /api/system/check-update`；系统先确认所有业务任务均已空闲，再启动稳定入口等待当前 `AI_Customer_App.exe` 正常关闭，随后执行与启动时相同的授权校验、签名校验、更新确认和进度显示。没有新版本时会提示“当前已是最新版本”并重新打开应用；开发环境或直接缺少 `AI_Customer.exe` 时会明确拒绝，不使用替代更新逻辑。页面顶部“刷新”仅刷新当前工作台；任何父级请求失败都会显示失败数量和首个错误，失败时不会把同步时间误记为成功，后台自动同步仍保持静默。打包版只有在 `/api/health` 完成数据库、授权和队列启动后才打开浏览器；重复双击 `AI_Customer.exe` 只重新打开已经运行的打包工作台，不会误连同机的开发服务，也不并发执行第二次更新。侧边栏“安全退出”调用 `POST /api/system/exit`，通过 Uvicorn lifespan 正常停止任务队列、调度器和浏览器会话；开发环境不显示该按钮。打包运行日志保存在 `data/logs/app.log`，单文件 5 MB 并保留三个轮转文件。以上能力涉及稳定入口改动，现有 `1.2.6` 交付包不包含；下一次明确要求打包时必须使用新的程序版本，不能覆盖 `deliverables/1.2.6/`。
