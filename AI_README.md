...
左侧栏底部提供“检查更新”按钮，并显示后端 `GET /api/health` 返回的当前程序版本。点击后前端调用 `POST /api/system/check-update`；系统先确认所有业务任务均已空闲，再启动稳定入口等待当前 `AI_Customer_App.exe` 正常关闭，随后执行与启动时相同的授权校验、签名校验、更新确认和进度显示。没有新版本时会提示“当前已是最新版本”并重新打开应用；开发环境或直接缺少 `AI_Customer.exe` 时会明确拒绝，不使用替代更新逻辑。该按钮涉及稳定入口改动，现有 `1.2.6` 交付包不包含此功能；下一次明确要求打包时必须使用新的程序版本，不能覆盖 `deliverables/1.2.6/`。
