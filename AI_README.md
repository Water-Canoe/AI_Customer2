...
6. 工作台顶部的“待处理队列”会汇总下一步事项：需补资料账号、竞品候选待分析、线索待筛选、目标客户待跟进和 AI 失败重试；点击队列项会直接跳转到总览树、数据表筛选或 AI 工作台。
