...
引流工作台已按四页重组：`计划工作台`、`执行监控`、`操作记录`、`引流设置`。旧的 `定向引流 / 随机引流` 不再作为独立页面，而是在计划工作台里作为来源模式：`随机推荐流`、`拓客竞品视频`、`已采集关键词`、`手动搜索关键词`。前端页面统一由 `frontend/src/pages/TrafficWorkbenchPage.ts` 渲染，继续复用拓客工作台的 `SplitPane`、`sectionTitle`、`table-workspace`、`data-table`、固定高度日志窗口和紧凑分页表格；计划列表、批次列表、视频明细、操作记录、文案库和图片库都按表格结构展示。
