...
当前产品版本唯一地定义在 `backend/app/version.py`，本轮为 `1.2.8`；`-Version` 省略时自动读取该值，显式传入不同值时构建会停止，避免 EXE 内版本与发布清单不一致。环境版本由构建参数独立管理，本轮为 `1.0.1`。只有明确要求正式打包时才执行：

```powershell
# 生成程序 ZIP 和环境 ZIP；程序版本默认读取 backend/app/version.py。
.\script\build_package.ps1 -Version 1.2.8 -EnvironmentVersion 1.0.1
```
