...
引流工作台已按四页重组：`计划工作台`、`执行监控`、`操作记录`、`引流设置`。旧的 `定向引流 / 随机引流` 不再作为独立页面，而是在计划工作台里作为来源模式：`随机推荐流`、`拓客竞品视频`、`已采集关键词`、`手动搜索关键词`。前端页面统一由 `frontend/src/pages/TrafficWorkbenchPage.ts` 渲染，继续复用拓客工作台的 `SplitPane`、`sectionTitle`、`table-workspace`、`data-table`、固定高度日志窗口和紧凑分页表格；计划列表、批次列表、视频明细、操作记录、文案库和图片库都按表格结构展示，计划列表和批次列表的操作列固定在右侧，横向滚动时仍可直接操作。执行监控里的“已处理视频”复用操作记录表字段，便于在批次详情和全局记录之间对照；操作记录页只保留筛选、表格和分页，清除记录这类危险操作统一放在引流设置页；日志采用状态色普通列表样式，默认自动跟随到底部，用户上滑查看历史时会暂停自动跟随。引流分栏页面现在用统一的外层卡片承载主区、拖拽条和右栏，表单、筛选栏、素材库、记录表都使用浅色块和紧凑表格，避免裸露分隔线或松散卡片让页面显得混乱。
...
第一版只实际执行抖音；计划工作台仍展示抖音、快手、小红书平台入口，点击快手或小红书只提示“正在开发”。创建计划时可留空计划名称，后端会按 `来源-计划ID前8位` 自动生成，例如 `拓客竞品视频-1a2b3c4d`，避免切换来源后仍沿用旧的随机推荐名称。动作组合包括点赞视频、收藏视频、关注作者、评论文案、评论图片；一个动作都不选时允许启动，表示“纯自动刷视频”，只随机停留、切换视频并写入“仅浏览”记录。文案和图片在引流设置页维护，文案库和图片库使用可增删改的表格编辑器，并支持启用/停用；执行时只从启用素材中随机抽取。引流设置页提供“操作执行概率”，默认 60%，每个已选择动作独立按该概率抽样执行；例如计划选择点赞和收藏时，单条视频可能同时执行两者，也可能只执行其中一个，全部被概率跳过时只写“仅浏览”。
