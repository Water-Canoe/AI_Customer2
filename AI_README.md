...
引流工作台已按四页重组：`计划工作台`、`执行监控`、`操作记录`、`引流设置`。旧的 `定向引流 / 随机引流` 不再作为独立页面，而是在计划工作台里作为来源模式：`随机推荐流`、`拓客竞品视频`、`已采集关键词`、`手动搜索关键词`。前端页面统一由 `frontend/src/pages/TrafficWorkbenchPage.ts` 渲染，继续复用拓客工作台的 `SplitPane`、`sectionTitle`、`table-workspace`、`data-table`、固定高度日志窗口和紧凑分页表格；计划列表、批次列表、视频明细、操作记录、文案库和图片库都按表格结构展示，计划列表和批次列表的操作列固定在右侧，横向滚动时仍可直接操作。执行监控里的“已处理视频”复用操作记录表字段，便于在批次详情和全局记录之间对照，并拥有独立滚动区，不再被日志和顶部指标挤压；操作记录页只保留筛选、表格和分页，清除记录这类危险操作统一放在引流设置页；日志采用状态色普通列表样式，默认自动跟随到底部，用户上滑查看历史时会暂停自动跟随。引流分栏页面现在用统一的外层卡片承载主区、拖拽条和右栏，表单、筛选栏、素材库、记录表都使用浅色块和紧凑表格，避免裸露分隔线或松散卡片让页面显得混乱。
