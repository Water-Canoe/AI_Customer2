...
from app.services import account_center, browser_queue, traffic_resources
...
def _run_with_playwright(run_id: str, plan: dict[str, Any]) -> dict[str, str]:
    ...
    settings = traffic_resources.get_settings()["values"]
...
def _validate_run_plan(plan: dict[str, Any]) -> None:
    ...
    settings = traffic_resources.get_settings()
...
def _format_record(row: Any) -> dict[str, Any]:
    ...
    preview_url = traffic_resources.image_preview_url(Path(str(data.get("comment_image_path") or "")))
    if preview_url:
