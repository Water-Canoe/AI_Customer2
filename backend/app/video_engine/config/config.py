...
config_file = str(Path(root_dir) / "config.default.toml")
...
project_name = _cfg.get("project_name", "AI Customer Video Engine")
...
def apply_runtime_config(values: dict) -> dict:
    ...
    for section_name, target in (
        ...
        target.clear()
        target.update(incoming.get(section_name, {}))

    log_level = incoming.get("log_level", "INFO")
    project_name = incoming.get("project_name", "AI Customer Video Engine")
    project_description = incoming.get("project_description", "")
    project_version = incoming.get("project_version", "1.3.1")
