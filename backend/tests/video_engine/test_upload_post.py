...
from app.video_engine.services.upload_post import UploadPostService
...
class TestUploadPostYouTube(unittest.TestCase):
    ...
    @patch("app.video_engine.services.upload_post.config.app", _CONFIG_BASE)
    @patch("app.video_engine.services.upload_post.os.path.exists", return_value=True)
    @patch("builtins.open", mock_open(read_data=b"fake"))
    @patch("app.video_engine.services.upload_post.requests.post")
    ...
    @patch("app.video_engine.services.upload_post.config.app", {
        **_CONFIG_BASE,
