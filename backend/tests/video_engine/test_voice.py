...
from app.video_engine.services import voice as vs
from app.video_engine.services import task as task_service
from pydub import AudioSegment

# 上游语音测试使用独立系统临时目录，不写入客户稳定data目录。
temp_dir = tempfile.mkdtemp(prefix="ai_customer_video_engine_voice_")
...
voice_rate=1.0
voice_volume=1.0
RUN_INTEGRATION_TESTS = os.environ.get("AI_CUSTOMER_VIDEO_INTEGRATION_TESTS", "").lower() in {
    "1",
    "true",
    "yes",
}
...
class TestElevenLabsVoice(unittest.TestCase):
    ...
    @patch("app.video_engine.services.voice.requests.get")
    ...
    @patch("app.video_engine.services.voice.requests.post")
    @patch("app.video_engine.services.voice.AudioFileClip")
    @patch("app.video_engine.services.voice.config")
