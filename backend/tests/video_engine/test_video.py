...
from app.video_engine.models.schema import MaterialInfo
from app.video_engine.services import state as sm
from app.video_engine.services import video as vd
...
class TestVideoService(unittest.TestCase):
    ...
    def test_get_bgm_file_rejects_old_project_relative_song_path(self):
        """
        移植后背景音乐只来自托管内容资产，旧项目resource相对路径不再接受。
        """
        song_dir = utils.song_dir()
        ...
        try:
            self.assertEqual(vd.get_bgm_file(bgm_file="./resource/songs/test-relative-bgm.mp3"), "")
        finally:
            if os.path.exists(bgm_path):
                os.remove(bgm_path)
